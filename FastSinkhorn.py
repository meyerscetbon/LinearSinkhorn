...
def Lin_Sinkhorn_RBF(X,Y,reg,a,b,num_samples,seed=49,delta=1e-9,num_iter=50,lam=1e-100):
    start = time.time()
...
def Nys_Sinkhorn_RBF_inv(X,Y,reg,a,b,rank,seed=49,delta=1e-9,num_iter=50,lam=1e-100):
    start = time.time()
...
def recursive_Nystrom_RBF(X,Y,rank,reg,seed=49,stable=1e-100):
    ...
    for l in range(nLevels-1,-1,-1):
        ...
        if k >= SKSn:
            ...
            lam = (np.sum(np.diag(SKS) * (weights**2)) - np.sum(np.abs(np.real(eigen))))/k
        ...
        if (l != 0):
            ...
            samp = np.random.choice(np.shape(levs)[0], rank, replace=False, p=levs_norm.reshape(-1))
