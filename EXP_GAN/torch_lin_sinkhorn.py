...
class Lin_Sinkhorn_AD(torch.autograd.Function):
    ...
    def forward(ctx, x_emb, y_emb, reg, niter_sin, lam=1e-6, tau=1e-9):
        ...
        u = 1.0 * torch.ones(n).type(torch.DoubleTensor)
        v = 1.0 * torch.ones(m).type(torch.DoubleTensor)
        err = 0.0
...
def Lin_Sinkhorn(
    phi_x, phi_y, reg, niter_sin, device, lam=1e-6, tau=1e-9, stabilize=False
):
    ...
    actual_nits = 0
    if stabilize == True:
        alpha, beta, err = torch.zeros(n).to(device), torch.zeros(m).to(device), 0.0
        for i in range(niter_sin):
            alpha_res = alpha
            beta_res = beta

            lin_M = torch.exp(alpha / reg) * torch.matmul(
                phi_x, torch.matmul(phi_y.t(), torch.exp(beta / reg))
            )
            lin_M = lin_M + lam
            alpha = reg * (torch.log(a) - torch.log(lin_M)) + alpha

            lin_M_t = torch.exp(beta / reg) * torch.matmul(
                phi_y, torch.matmul(phi_x.t(), torch.exp(alpha / reg))
            )
            lin_M_t = lin_M + lam
        ...
        u = 1.0 * torch.ones(n).type(torch.DoubleTensor).to(device)
        v = 1.0 * torch.ones(m).type(torch.DoubleTensor).to(device)
        err = 0.0
