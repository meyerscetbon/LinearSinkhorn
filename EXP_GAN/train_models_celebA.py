...
def training_func(num_random_samples,reg,batch_size,niter_sin,image_size,nc,nz,dataset_name,device,manual_seed,lr,max_iter,data_root,R,random_):
    ...
    file_to_print = open("results_training_celebA"+'_'+str(num_random_samples)+'_'+str(reg)+".csv", "w")
    ...
    for t in range(max_iter):
        ...
        if t % 10 == 0:
            torch.save(netG.state_dict(), 'netG_celebA'+'_'+str(num_random_samples)+'_'+str(reg)+'.pth')
            torch.save(netE.state_dict(), 'netE_celebA'+'_'+str(num_random_samples)+'_'+str(reg)+'.pth')
