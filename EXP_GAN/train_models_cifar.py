...
def training_func(num_random_samples,reg,batch_size,niter_sin,image_size,nc,nz,dataset_name,device,manual_seed,lr,max_iter,data_root,R,random_):

    name_dir = 'sampled_images_cifar_max'+'_'+str(num_random_samples)+'_'+str(reg)
    ...
    file_to_print = open("results_training_cifar_max"+'_'+str(num_random_samples)+'_'+str(reg)+".csv", "w")
    ...
    trn_dataset = data_loading.get_data(image_size,dataset_name,data_root, train_flag=True)
    ...
    for t in range(max_iter):
        ...
        while (i < len(trn_loader)):
            ...
            for j in range(Diters):
                ...
                ### Compute the loss ###
                sink_E = 2 * lin_Sinkhorn_AD(x_emb, y_emb, epsilon, niter_sin) \
            ...
            for j in range(Giters):
                ...
                # Compute the loss
                sink_G = 2 * lin_Sinkhorn_AD(x_emb, y_emb, epsilon, niter_sin) \
                - lin_Sinkhorn_AD(y_emb, y_emb, epsilon, niter_sin) \
                - lin_Sinkhorn_AD(x_emb, x_emb, epsilon, niter_sin)
            ...
            s = "[%3d / %3d] [%3d / %3d] [%5d] (%.2f m) loss_E: %.6f loss_G: %.6f" % (
                    t,max_iter, i * batch_size, batch_size * len(trn_loader),
                    gen_iterations,
                    run_time,
                    sink_E,
                    sink_G,
            ...
            if gen_iterations % 100 == 0:
                with torch.no_grad():
                    ...
                    vutils.save_image(y_fixed, '{0}/fake_samples_{1}.png'.format(name_dir, gen_iterations))


        if t % 10 == 0:
            torch.save(netG.state_dict(), 'netG_cifar_max'+'_'+str(num_random_samples)+'_'+str(reg)+'.pth')
            torch.save(netE.state_dict(), 'netE_cifar_max'+'_'+str(num_random_samples)+'_'+str(reg)+'.pth')
...
if __name__ == '__main__':
    for num_random_samples in num_random_samples_list:
        for reg in reg_list:
            training_func(num_random_samples,reg,batch_size,niter_sin,image_size,nc,nz,dataset_name,device,manual_seed,lr,max_iter,data_root,R,random_)
